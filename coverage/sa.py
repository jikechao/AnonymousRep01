...
def get_ats(
    ...
    # print("get_ats",len(dataset),dataset.shape)
    temp_model = Model(
        inputs=model.input,
...
def _get_train_target_ats(model, x_train, x_target, target_name, layer_names, args):
    ...
    if os.path.exists(saved_train_path[0]):
        ...
        # print("get_train_target_ats", x_train.shape)
...
def get_sc(lower, upper, k, sa):
    ...
    # print(list(set(buckets)))
    # print("sc",list(set(buckets)),len(list(set(buckets))),k,float(k),len(list(set(buckets))) / float(k))
