...
if __name__ == "__main__":
    ...
    if args.dsa:
        ...
        target_dsa = fetch_dsa(model, x_train, x_test[:10], args.target, layer_names, args)
        target_cov, buckets = get_sc(
            0, 1, args.n_bucket, target_dsa
        )
        ...
        for i in range(len(buckets)):
            ...
            for j in range(len(buckets)):
                temp.append(abs(buckets[i]-buckets[j]))
            temp = np.array(temp)
            temp_all.append(np.mean(temp))
        temp_all = np.array(temp_all)
        diversity = np.mean(temp_all)
